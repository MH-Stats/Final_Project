...
# Grabbing best model
best_subc_param <- subc_fit |>
  select_best(metric = "mn_log_loss")

# Final fit
subc_final_fit <- subc_wkflw |>
  finalize_workflow(best_subc_param) |>
  last_fit(sub_split, metrics = metric_set(mn_log_loss))
```
...
# Creating subsetted workflow
subrf_wkflw <- workflow() |>
  add_recipe(subc_recipe) |> # same recipe from classification tree will work here
  add_model(rf_spec)
...
```{r}
# fitting subsetted data with tune_grid and grid_regular
subrf_fit <- subrf_wkflw |>
  tune_grid(resamples = sub_cv,
             grid = 10,
             metrics = metric_set(mn_log_loss))
...
# Grabbing best one
best_subrf_param <- subrf_fit |>
  select_best(metric = "mn_log_loss")

# Using finalize_workflow to create final model
subrf_final <- subrf_wkflw |>
  finalize_workflow(best_subrf_param) |>
  last_fit(sub_split, metrics = metric_set(mn_log_loss))
```
